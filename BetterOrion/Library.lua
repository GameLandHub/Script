...
function OrionLib:MakeWindow(WindowConfig)
	...
	local function SetMouseUnlocked(state)
		...
		if state then
			-- Quando o mouse está desbloqueado, faz um loop para manter
			task.spawn(function()
	...
	-- Aplica o FreeMouse inicial se configurado
	...
	-- Local window connections
		AddConnection(CloseBtn.MouseButton1Up, function()
			...
			-- Desbloqueia o mouse quando a UI é fechada
			if WindowConfig.FreeMouse then
		...
		AddConnection(UserInputService.InputBegan, function(Input)
			if Input.KeyCode == WindowConfig.ToggleUIKey then 
				...
				-- Alterna o estado do mouse conforme a visibilidade da UI
				if WindowConfig.FreeMouse then
